...
global_min =loweredge_weights.min() #upperedge_weights.min()
global_max =loweredge_weights.max()#upperedge_weights.max()
...
# PLOT 4: SPECTRUM COLORED BY LOWER EDGE LOCALIZATION

'''plt.figure(figsize=(6,6))

for i in range(2*Ly):
    plt.scatter(kx_vals, energies[:, i],
                c=loweredge_weights[:, i],
                cmap='viridis',norm=norm , s=5)

plt.colorbar(label=r"$|\langle \psi(n=2Ly-1)|eigvecs \rangle|^2$")
plt.xlabel(r"$k_x$")
plt.ylabel("Energy")
plt.title(f"Finite Sample Band Spectrum with Lower Edge Localization(t1={t1},t2={t2},M={M},phi={phi:.2f})")
#plt.ylim(-3, 3)
plt.show()'''
